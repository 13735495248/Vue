...
邮箱注册:https://mail.163.com/register/index.htm?from=force/&cmd=register.entrance
...
WXS存在的意义就是充当computed/filter这类功能。wxs不支持es6语法。
...
```javascript
<!--
bindsubmit：提交表单的时候会触发的方法(会自动入参事件对象e，在e中就包含了表单的数据)
-->
<form bindsubmit="submitForm">
    ...
    <view>
        ...
        <button type="primary" formType="reset">重置表单</button>
    </view>
</form>
...
Page({
  ...
  hobbyChange(e){
    console.log("hobbyChange",e.detail.value)
  },
  submitForm(e){
...
```javascript
//1.在父组件中声明一个方法用于接收子组件的数据
//2.在父组件中使用子组件的时候把这个方法传递给子组件
<Tabs tabs="{{tabs}}" binditemChange="handleItemChange"></Tabs>
//3.在子组件的点击方法中通过triggerEvent触发父组件传递过来的事件
hanldeItemTap(e){
...
2.Vant WeApp：有赞移动 Web UI 规范 ZanUI 的小程序实现版本，结合了微信的视觉规范，为用户提供更加统一的使用感受。现已包含 badge、btn、card、cell、dialog、icon、label、noticebar、panel、popup、switch、tab、toast、tooltips 等组件或元素。
   GitHub地址：https://youzan.github.io/vant-weapp/#/cell

3.MinUI ：MinUI 是蘑菇街前端开发团队开发的基于微信小程序自定义组件特性开发而成的一套简洁、易用、高效的组件库，适用场景广，覆盖小程序原生框架，各种小程序组件主流框架等，并且提供了专门的命令行工具。
GitHub地址：https://github.com/meili/minui

4.Wux WeApp：Wux WeApp也是一个非常不错的微信小程序自定义 UI 组件库，组件比较丰富，值得使用。
GitHub地址：https://github.com/wux-weapp/wux-weapp npm下载：npm i wux-weapp

5.ColorUI：ColorUI是一个Css类的UI组件库！不是一个Js框架。相比于同类小程序组件库，ColorUI更注重于视觉交互！其组件在美观性方面比较突出。
...
"usingComponents": {
    "vant-button": "../../dist/button/index"
}

3. 在 wxml 中使用组件：

<vant-button type="primary" bind:click="handleClick">这是一个按钮</vant-button>
```
...
```javascript
 <!-- 
  ...
  3 getPhoneNumber 获取当前用户的手机号码信息 结合一个事件来使用  不是企业的小程序账号没有权限来获取用户的手机号码 
    ...
    c) 获取到的信息  已经加密过了 ，需要开发者搭建自己的后台服务器，然后在第三方服务端结合 session_key 以及 app_id 进行解密获取手机号。
  4 getUserInfo 获取当前用户的个人信息
    a) 使用方法 类似 获取用户的手机号码
    b) 可以直接获取 不存在加密的字段 
  5 launchApp 在小程序当中 直接打开 app
    a) 需要在 app中 通过分享的某个链接 打开 小程序
...
//getUserProfile来获取用户基本信息  该接口用于替换 wx.getUserInfo
wx.getUserProfile({
    desc: '展示',
    success: (res) => {
...
https://mp.weixin.qq.com/s?__biz=MzUxNzk1MjQ0Ng==&mid=2247488824&idx=1&sn=96dba0f848af5342b37708b33473dcee&chksm=f99113e9cee69affd53fe60e01d4313052dcd49d8118506550610002cb0cb6f0c2b66141777e&mpshare=1&scene=2&srcid=1203M2KowPwkp1iiyxI1hSuC&sharer_sharetime=1607038124150&sharer_shareid=59da7d7e1eb3747a310dbb4cdf1fcacc#rd
...
//关于支付：
//1.必须要企业账号才有支付权限
//2.开发过程中，必须在企业账号小程序后台中添加白名单
//3.要实现支付功能，必须要自己搭建App的后台服务器，在后台服务器中调用微信支付接口
```
...
//获取用户运动步数
  handleChooseRun() {
    ...
    wx.getSetting({
      success: (result) => {
        //获取用户之前对于步数的操作
        const scopeRun = result.authSetting["scope.werun"];
        //如果从来没有点击过或者同意用户步数的授权
        if (scopeRun === true || scopeRun === undefined) {
          //获取用户运动步数
          wx.getWeRunData({
            success: (result1) => {
              console.log(result1, "6666666666666666666");
            },
          });
        }
        //如果用户之前点击了拒绝
          ...
          wx.openSetting({
            success: (result2) => {
              ...
              wx.getWeRunData({
                success: (result3) => {
                  console.log(result3, "6666666666666666");
                },
              });
            },
          });
...
```javascript
async handleChooseRun2() {
    try {
      let settings = await getSetting();
      const scopeRun = settings.authSetting["scope.werun"];
      if (scopeRun === false) {
        let opensettings = await openSetting();
      } 
      let weiRunData = await getWeRunData();
      console.log(weiRunData, "8888888888888888");
    } catch (error) {
      console.log(error);
    }
  },
...
```javascript
  onLoad: function (options) {
    ...
    requestGet(`/categories/${options.category}`).then(res => {
...
Page({
  ...
  //获取列表数据
  async getListData() {
    let result = await requestGet(`/categories/${this.data.id}/shops`, { _page: this.data.pageIndex, _limit: this.data.pageSize })
    this.setData({
      shopList: [...this.data.shopList, ...result.data],
      totalCount: parseInt(result.header['x-total-count'])
    })
  },
  //searchbar在输入文字的时候触发的事件
  search: function (value) {
    //1.获取到当前页面的数据源
    var list = this.data.shopList;
    //2.search方法需要返回一个Promise
    return new Promise((resolve, reject) => {
      //如果用户没有输入内容，就返回一个空数组
      if (value.length == 0) {
        resolve([])
      ...
      //如果用户输入了内容，就根据用户输入的内容从数据源中查找指定的内容
...
Page({
  ...
  onReachBottom: function () {
    ...
    //3.判断当前的pageIndex是否小于等于总页数，发送请求
...
<navigator wx:for="{{ shopList }}" wx:key="unique" url="{{'/pages/detail/detail?id='+item.id}}">
    ......
</navigator>


#2.   pages/detail/detail.js
Page({
  data: {
    id: 0,
    shop:{}
...
Page({
  ...
  preview(e){
    //图片预览，current表示当前我要预览的图片，urls表示所有预览图片的链接
    wx.previewImage({
...
1、订阅号
2、服务号
3、企业号：企业号多半是企业内容使用，用于管理员工通讯，方便员工通讯，信息集合和消息通知等功能，相当于一个移动的小型oa系统了。一般不用来对外宣传推广
```
